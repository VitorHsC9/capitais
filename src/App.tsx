...
import { SupremeCapitals } from './components/SupremeCapitals';
import { SupremeCountries } from './components/SupremeCountries';
import { SupremeFinal } from './components/SupremeFinal';
        ...
        <main className={`flex-1 flex flex-col py-4 px-4 overflow-hidden relative transition-all duration-300 w-full`}>
          ...
          {screen === 'supreme-menu' && (
            <SupremeMenu
              ...
              onSelectCapitals={() => setScreen('supreme-capitals')}
              onSelectCountries={() => setScreen('supreme-countries')}
              onSelectFinal={() => setScreen('supreme-final')}
            />
          )}

          {/* SUPREME CAPITALS */}
          {screen === 'supreme-capitals' && (
            <SupremeCapitals onBack={() => setScreen('supreme-menu')} />
          )}

          {/* SUPREME COUNTRIES */}
          {screen === 'supreme-countries' && (
            <SupremeCountries onBack={() => setScreen('supreme-menu')} />
          )}

          {/* SUPREME FINAL */}
          {screen === 'supreme-final' && (
            <SupremeFinal onBack={() => setScreen('supreme-menu')} />
          )}
          ...
          {screen === 'practice' && (
            <div className="flex flex-col h-full">
              <div className="flex items-center gap-3 mb-2">
                <button onClick={() => setScreen('home')} className="btn-neutral w-10 h-10 rounded-xl">
          ...
          {screen === 'continents' && (
            <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="flex items-center gap-3 mb-6">
                <button onClick={() => setScreen('practice')} className="btn-neutral w-10 h-10 rounded-xl">
                  <ChevronLeft className="w-6 h-6" />
                </button>
                <div>
          ...
          {screen === 'playing' && (
            <div className="flex flex-col h-full gap-4 animate-in fade-in duration-300">
              ...
              <div className="flex items-center justify-between bg-[var(--surface-color)] p-3 rounded-xl border-2 border-[var(--border-color)]">
                <button onClick={() => { game.restart(); setScreen('practice'); }} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)] hover:text-[var(--color-error)] transition-colors">
