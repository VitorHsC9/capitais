    ...
    if (gameStatus === 'lost') {
        return (
            <div className="flex flex-col h-full items-center justify-center p-6 animate-in fade-in duration-500">
                ...
                <p className="text-[var(--text-secondary)] font-bold mb-8 text-center">Você conseguiu acertar {progress} de {totalCountries} capitais.</p>
            ...
            <div className="flex flex-col gap-4 mb-4 z-10 relative bg-[var(--bg-color)] pb-2">
                <div className="flex items-center justify-between">
                    ...
                    <div className="text-center">
                        <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--text-secondary)]">Supremo Capitais</h2>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-3 rounded-xl bg-[var(--surface-color)] border-2 border-[var(--border-color)] text-[var(--text-primary)] font-bold placeholder:text-[var(--text-secondary)] focus:outline-none focus:border-[var(--color-secondary)] transition-colors uppercase"
                        autoFocus
                    />
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {countries.map(country => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className="font-bold text-sm text-[var(--text-primary)]">{country.name}</span>
                                        {isGuessed ? (
                                            <span className="font-black text-sm text-[var(--color-secondary)] uppercase animate-in fade-in slide-in-from-right-2">
                                                {country.capital}
                                            </span>
                                        ) : (
                                            <div className="h-4 w-12 bg-[var(--border-color)]/30 rounded animate-pulse"></div>
                                        )}
                                    </div>
                                );
                            })}
